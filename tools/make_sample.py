...
LINK_TIMEOUT_S = 3.0
PRELOAD = 16
WALL0 = dt.datetime(2026, 9, 19, 20, 30, 0, tzinfo=dt.timezone.utc)

NODE_PLAN = [
    dict(node_id=0, label="SAT-1", transport="/dev/ttyUSB0", first=1.4, interval=8.6, jitter=1.6, link_kbps=26.0,
         scenes=["great_lakes", "iowa", "tokyo", "sahara", "atacama"], cloudy=6),
    dict(node_id=1, label="SAT-2", transport="/dev/ttyUSB1", first=2.1, interval=8.0, jitter=1.2, link_kbps=30.0,
         scenes=["cairo", "amazon", "hawaii", "ganges"], cloudy=2),
    dict(node_id=2, label="SAT-3", transport="/dev/ttyUSB2", first=2.9, interval=9.0, jitter=1.8, link_kbps=21.0,
         scenes=["greenland", "alps", "netherlands", "himalaya", "pacific"], cloudy=11),
]
OUTAGE = dict(node_id=2, start=47.5, length=13.4)
CRC_WARN = dict(node_id=0, t=71.3, message="3 crc errors in last 10s")
...
def load_frames():
    ...
    for i in c.ids:
        ...
        out[i] = dict(frame_id=i, scene=c.scene_of(i), is_ref=(i == ref), score=s.score, clear=s.clear, sharp=s.sharp,
                      change=s.change, cloud_frac=round(s.cloud_px / 16384, 4))
    return out


def parts_of(f: dict) -> dict:
...
class Node:
    def __init__(self, plan: dict, frames: list[dict], rng: random.Random):
        ...
        self.busy = False
        self.link = True
        self.buffered: list[dict] = []       # frame_scored payloads held while the link is down
        self.ground_view = None              # (queue copy, n_scored, n_evicted, n_sent) frozen when the link dropped
        self.rng = rng

    def insert(self, f: dict, t: float) -> tuple[bool, int | None]:
    ...
    def window(self, t: float) -> list[dict]:
        return [{"frame_id": fid, "score": s, "age_s": round(t - ts, 1)} for s, fid, ts in self.queue[:5]]


class Sim:
    def __init__(self, seed: int, frames_by_id: dict):
        ...
        self.used = 0
        self.slot = 0
        self.open_grant = None            # dict(node, slot, frame, t)
        self.next_grant_ok = 0.0
        self.winners: list[int] = []
        self.arrivals: list[dict] = []
        self.closed = False
        # baseline (FIFO, round robin, same limit)
        self.fifo: dict[int, list[dict]] = {n.id: [] for n in self.nodes}
        self.fifo_dropped = 0
        self.rr = 0
        self.base: list[dict] = []
    ...
    # -- plumbing
    def emit(self, t: float, typ: str, **fields):
        t = round(max(t, self.last_t + 0.001), 3)
        self.last_t = t
        self.seq += 1
        ev = {"seq": self.seq, "t": t, "type": typ}
        ev.update(fields)
        ev["wall"] = (WALL0 + dt.timedelta(seconds=t)).strftime("%Y-%m-%dT%H:%M:%S.") + f"{int(round(t * 1000)) % 1000:03d}Z"
    ...
    # -- schedule
    def start(self, t):
        self.emit(0.0, "run_start", run_id="sample-2026-09-19T20-30-00", mode="live",
                  nodes=[{"node_id": n.id, "label": n.label, "transport": n.plan["transport"], "real": True} for n in self.nodes],
                  window={"budget_bytes": BUDGET, "duration_s": DURATION}, queue_limit=QUEUE_LIMIT, scoring=SCORING,
                  usable_rule={"metric": "cloud_frac", "max": USABLE_MAX})
        for k, n in enumerate(self.nodes):
            self.emit(0.2 + 0.15 * k, "node_event", node_id=n.id, level="info",
        ...
        self.at(CRC_WARN["t"], lambda t: self.emit(t, "node_event", node_id=CRC_WARN["node_id"], level="warn",
                                                  message=CRC_WARN["message"]))
    ...
    def status(self, t, n: Node, link_ok=True):
        queue, n_scored, n_evicted, n_sent = (n.queue, n.n_scored, n.n_evicted, n.n_sent) if link_ok else n.ground_view
        h = queue[0] if queue else None
        self.emit(t, "node_status", node_id=n.id, queue_depth=len(queue), top_frame_id=h[1] if h else None,
                  top_score=h[0] if h else 0, top_age_s=round(t - h[2], 1) if h else 0.0, frames_scored=n_scored,
                  frames_evicted=n_evicted, frames_sent=n_sent, busy=n.busy, link_ok=link_ok)

    def queue_window(self, t, n: Node):
        self.emit(t, "queue_window", node_id=n.id, depth=len(n.queue), top=n.window(t))

    def score_next(self, t, nid):
        n = self.by_id[nid]
        ...
        n.n_scored += 1
        payload = dict(node_id=n.id, frame_id=f["frame_id"], score=f["score"], parts=parts_of(f),
                       cloud_frac=f["cloud_frac"], queued=queued, evicted_frame_id=evicted, queue_depth=len(n.queue))
        if n.link:
    ...
    def report_scored(self, t, n: Node, payload: dict):
    ...
    def window_update(self, t):
        remaining = BUDGET - self.used
        open_ = remaining >= FRAME_BYTES and t < DURATION
        if not open_:
            self.closed = True
        self.emit(t, "window_update", budget_bytes=BUDGET, used_bytes=self.used, remaining_bytes=remaining,
                  time_remaining_s=round(DURATION - t, 1), open=open_)

    def arbitrate(self, t):
        if not self.closed and self.open_grant is None and t >= self.next_grant_ok:
            bids = [{"node_id": n.id, "top_score": n.head()[0] if (n.link and n.queue) else 0,
                     "ready": bool(n.link and n.queue)} for n in self.nodes]
            ready = sorted((b for b in bids if b["ready"]), key=lambda b: (-b["top_score"], b["node_id"]))
            if ready:
                best = ready[0]
                if len(ready) == 1:
                    winner, reason = best, "only_ready"
                elif len(self.winners) >= STARVATION_N and all(w == best["node_id"] for w in self.winners[-STARVATION_N:]):
    ...
    def grant(self, t, n: Node, reason: str, bids: list[dict]):
        self.slot += 1
        score, fid, _ = n.queue.pop(0)               # the node pops its head on GRANT and starts sending
        n.busy = True
        self.winners.append(n.id)
        self.emit(t, "grant", slot_id=self.slot, node_id=n.id, budget_bytes=min(FRAME_BYTES, BUDGET - self.used),
                  reason=reason, bids=bids)
    ...
    def arrive(self, t, duration):
        ...
        ev = self.emit(t, "frame_arrived", slot_id=g["slot"], node_id=n.id, frame_id=f["frame_id"], score=f["score"],
                       bytes=FRAME_BYTES, duration_s=duration, cloud_frac=f["cloud_frac"], usable=f["cloud_frac"] <= USABLE_MAX)
    ...
    def baseline(self, t):
        for k in range(len(self.nodes)):
            ...
            if self.fifo[nid]:
                ...
                ev = self.emit(t, "baseline_arrival", node_id=nid, frame_id=f["frame_id"], score=f["score"],
                               bytes=FRAME_BYTES, cloud_frac=f["cloud_frac"], usable=f["cloud_frac"] <= USABLE_MAX)
                self.base.append(ev)
                return

    def outage_begin(self, t):
        ...
        self.at(t + 2.0, lambda t2: self.emit(t2, "node_event", node_id=n.id, level="warn",
                                              message="heartbeat missed x2 (2.0 s), retrying"))

        def timeout(t3):
            self.emit(t3, "node_event", node_id=n.id, level="error",
    ...
    def outage_end(self, t):
        n = self.by_id[OUTAGE["node_id"]]
        n.link = True
        k = len(n.buffered)
        self.emit(t, "node_event", node_id=n.id, level="info",
                  message=f"link restored after {OUTAGE['length']:.1f} s, {k} buffered frame report{'s' if k != 1 else ''} received")
    ...
    def totals(self) -> dict:
        ...
        orbit = dict(frames_down=len(self.arrivals), usable_down=usable, bytes_used=self.used,
                     frames_left_queued=sum(len(n.queue) for n in self.nodes))
        base = dict(frames_down=len(self.base), usable_down=b_usable, bytes_used=len(self.base) * FRAME_BYTES,
                    frames_left_queued=sum(len(q) for q in self.fifo.values()))
        gain = round(usable / b_usable, 2) if b_usable else None
        return dict(orbit=orbit, baseline=base,
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--seed", type=int, default=8, help="8 is the committed sample; 39 also has frames rejected by a full queue")
    ...
    if args.debug:
        for n in sim.nodes:
            print(f"{n.label} pool: " + " ".join(f"#{f['frame_id']}:{f['score'] * 100 / 65535:.0f}{'c' if f['cloud_frac'] > USABLE_MAX else ''}"
                                                 for f in n.frames))
        for a in sim.arrivals:
            ...
            print(f"slot {a['slot_id']:2d} t={g['t']:6.1f} -> {sim.by_id[a['node_id']].label} #{a['frame_id']:<3d} "
                  f"{a['score'] * 100 / 65535:5.1f} {'usable' if a['usable'] else 'CLOUDY'}  {g['reason']:17s} bids {bids}")
        print("baseline: " + " ".join(f"{sim.by_id[b['node_id']].label[-1]}#{b['frame_id']}{'' if b['usable'] else 'c'}" for b in sim.base))
