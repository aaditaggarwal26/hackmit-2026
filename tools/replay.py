...
class State:
    ...
    def reset(self):
        ...
        self.n = 0
        self.run_start = None
        self.latest: dict = {}      # (type, node_id) -> (n, event)
        self.log: list = []         # (n, event) for everything kept in full
...
def describe(ev: dict) -> str:
    ...
    try:
        if typ == "run_start":
            ...
            w = g("window", {})
            return f"run {g('run_id')} mode={g('mode')} nodes=[{nodes}] budget={w.get('budget_bytes')} B duration={w.get('duration_s')} s queue_limit={g('queue_limit')}"
        if typ == "frame_scored":
            return f"node {g('node_id')} frame {g('frame_id')} score {g('score')} cloud {g('cloud_frac')} queued={g('queued')} evicted={g('evicted_frame_id')} depth={g('queue_depth')}"
        if typ == "grant":
            bids = " ".join(f"{b.get('node_id')}:{b.get('top_score')}{'' if b.get('ready') else '(not ready)'}" for b in g("bids", []))
            return f"slot {g('slot_id')} -> node {g('node_id')}  {g('reason')}  bids {bids}"
        if typ == "frame_arrived":
            return f"slot {g('slot_id')} node {g('node_id')} frame {g('frame_id')} score {g('score')} {'USABLE' if g('usable') else 'not usable'} {g('bytes')} B in {g('duration_s')} s"
        if typ == "baseline_arrival":
            return f"FIFO would have sent node {g('node_id')} frame {g('frame_id')} score {g('score')} {'usable' if g('usable') else 'not usable'}"
        if typ == "window_update":
            return f"used {g('used_bytes')}/{g('budget_bytes')} B  remaining {g('remaining_bytes')}  {g('time_remaining_s')} s left  open={g('open')}"
        if typ == "node_event":
            return f"node {g('node_id')} {str(g('level')).upper()} {g('message')}"
        if typ == "run_end":
            o, b, h = g("orbit", {}), g("baseline", {}), g("headline", {})
            return f"orbit {o.get('usable_down')}/{o.get('frames_down')} usable, baseline {b.get('usable_down')}/{b.get('frames_down')}, gain {h.get('gain')}"
        if typ == "node_status":
            return f"node {g('node_id')} depth {g('queue_depth')} top {g('top_frame_id')}@{g('top_score')} busy={g('busy')} link={g('link_ok')}"
        if typ == "queue_window":
...
# ---------------------------------------------------------------- server
class Replayer:
    def __init__(self, path: Path, speed: float, loop: bool, start: float, host: str, port: int,  # noqa: PLR0913
                 frames_dir: Path, html: Path, verbose: bool):
        ...
        self.run_id = next((str(e["run_id"]) for e in self.events
                            if e.get("type") == "run_start" and e.get("run_id") is not None), None)
        ...
        while True:
            ...
            for idx, ev in enumerate(self.events[i:], i):
                ...
                if self.verbose or ev.get("type") not in QUIET_TYPES:
                    print(f"[t={self.position:8.3f}] #{str(ev.get('seq')):>4} {str(ev.get('type')):16s} {describe(ev)}", flush=True)
            if not self.loop:
    ...
    def drop(self, w: asyncio.StreamWriter):
    ...
    def snapshot(self) -> dict:
        return {"source": {"kind": "replay", "file": self.path.name, "speed": self.speed, "loop": self.loop,
                           "pass": self.pass_no, "playing": self.playing, "position_s": self.position,
                           "duration_s": self.t_last, "events_total": len(self.events), "clients": len(self.clients)},
                "seq": self.state.seq, "t": self.state.t, "events": self.state.events()}
    ...
    async def handle(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        ...
        try:
            if path == "/ws" and "websocket" in headers.get("upgrade", "").lower():
                ...
                await self.respond_file(writer, self.html, fallback=b"<!doctype html><meta charset=utf-8><title>replay</title>"
                ...
                if "/" in name or name.startswith(".") or not name.replace("-", "").replace("_", "").replace(".", "").isalnum():
            ...
            elif self.static_file(path) is not None:                # fonts/ and anything else next to the page
                await self.respond_file(writer, self.static_file(path), cache="max-age=3600")
            else:
    ...
    def list_runs(self) -> list:
        ...
        for p in sorted(self.path.parent.glob("*.jsonl"), key=lambda q: q.stat().st_mtime, reverse=True):
            ...
            out.append({"file": p.name, "run_id": start.get("run_id") if start else None,
                        "mode": start.get("mode") if start else None, "events": len(evs),
                        "ended": end is not None, "ended_t": end.get("t") if end else None,
    ...
    async def respond(self, writer, status: int, body: bytes, ctype: str, cache: str = "no-store"):
    ...
    async def respond_file(self, writer, p: Path, fallback: bytes | None = None, cache: str = "no-store"):
        try:
            body = p.read_bytes()
        except OSError:
            if fallback is not None:
                ...
                await self.respond(writer, 404, b"not found", "text/plain")
            return
        ctype = mimetypes.guess_type(p.name)[0] or {".woff2": "font/woff2", ".woff": "font/woff",
                                                    ".jsonl": "application/x-ndjson"}.get(p.suffix, "application/octet-stream")
    ...
    async def serve(self):
        ...
        print(f"replay  {self.path}  {len(self.events)} events  t {self.t_first:.1f} -> {self.t_last:.1f} s"
              f"  speed {self.speed:g}x  loop {'on' if self.loop else 'off'}" + (f"  start {self.start:g} s" if self.start else ""))
        print(f"serving ws://{self.host}:{self.port}/ws   http://{self.host}:{self.port}/  ({self.html.relative_to(REPO) if self.html.exists() else 'page missing'})"
              f"   /api/state   /frames/ -> {self.frames_dir.relative_to(REPO) if self.frames_dir.exists() else 'none'}"
              f"   /runs/{self.run_id or self.path.stem}.jsonl (as recorded so far)   /api/runs", flush=True)
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--pause", type=float, default=LOOP_PAUSE_S, help=f"seconds to hold the finished run before looping (default {LOOP_PAUSE_S:g})")
    ap.add_argument("--start", type=float, default=0.0, help="begin at this t; earlier events are applied to state silently")
