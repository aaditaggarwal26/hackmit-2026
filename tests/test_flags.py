...
S = config.Settings(
    soft_wait_s=20.0,
    hard_wait_s=60.0,
    memory_pressure_evictions_per_min=6.0,
    memory_pressure_window_s=60.0,
    peer_stale_s=5.0,
)
...
def rec(*, first=0.0, seen=None, last_tx=None, qlen=1, evictions=0, now=0.0):
    r = SatelliteRecord(
        "sat-x",
        first_seen_s=first,
        last_seen_s=seen if seen is not None else now,
        last_tx_complete_s=last_tx,
        queue_len=qlen,
    )
