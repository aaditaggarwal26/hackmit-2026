...
from orbit.golden.score import Config, display, score_frame  # noqa: E402

H, W, NPIX = params.FRAME_H, params.FRAME_W, params.FRAME_BYTES
...
class Session:
    ...
    def __init__(self, limit: int = QUEUE_LIMIT, cfg: Config = Config()):
    ...
    def score(self, frame: np.ndarray) -> dict:
        ...
        shot = dict(frame_id=fid, score=s.score, display=round(display(s.score), 1),
                    cloud_frac=round(cloud_frac, 4), usable=cloud_frac <= USABLE_MAX,
                    thumb=png(frame))
        self.shots[fid] = shot
        self.ref, self.ref_id = frame, fid
        return dict(
            **{k: v for k, v in shot.items() if k != "thumb"},
            first=first, ref_id=ref_id,          # the photo this one was compared against
            ...
            queued=not rejected, rejected=rejected, evicted_frame_id=evicted,
            maps=dict(frame=shot["thumb"],
                      cloud=png(paint(frame, frame > cfg.cloud_thr, (192, 39, 31))),
                      edges=png(edge_map(frame)),
                      change=png(paint(frame, np.abs(frame.astype(np.int16) - ref.astype(np.int16)) > cfg.change_thr,
                                       (27, 84, 200)))),
            **self.state())

    def state(self) -> dict:
        return dict(depth=len(self.queue), limit=self.limit, evicted_total=self.queue.evicted,
                    queue=[dict(self.shots[i], rank=k) for k, (_, i) in enumerate(self.queue.cells)])

    def config(self) -> dict:
        c = self.cfg
        return dict(w_clear=c.w_clear, w_sharp=c.w_sharp, w_change=c.w_change, cloud_thr=c.cloud_thr,
                    change_thr=c.change_thr, sharp_shift=c.sharp_shift, queue_limit=self.limit,
                    frame=dict(w=W, h=H, bytes=NPIX), usable_rule=dict(metric="cloud_frac", max=USABLE_MAX),
                    kernel="orbit/golden/score.py", **self.state())
...
class Handler(BaseHTTPRequestHandler):
    ...
    def score(self) -> None:
        ...
        try:
            ...
            return
        print(f"photo #{out['frame_id']:<3d} score {out['display']:5.1f}/100  cloud {out['cloud_frac'] * 100:5.1f}%  "
              f"{'usable' if out['usable'] else 'NOT USABLE'}  "
              f"{'queued' if out['queued'] else 'rejected, queue full'}"
              f"{f', pushed out #{out['evicted_frame_id']}' if out['evicted_frame_id'] else ''}", flush=True)
        self.json(out)
...
def main(argv=None) -> int:
    ...
    Handler.session = Session(max(1, min(a.queue, params.QUEUE_DEPTH)))
    try:
        httpd = ThreadingHTTPServer((a.host, a.port), Handler)
    except OSError as e:                          # something else has the port: say which, not a traceback
        print(f"! port {a.port} is busy ({e.strerror or e}). Something is already serving there - the display\n"
              f"  replayer uses 8000. Pick another: uv run python camera/demo.py --port {a.port + 1}", file=sys.stderr)
        return 1
    print(f"orbit camera demo   http://{a.host}:{a.port}/\n"
          ...
          f"Chrome will ask for the camera the first time you open the page. Ctrl-C to stop.", flush=True)
    try:
        httpd.serve_forever()
