## Orbit edge node — Arty A7-100T constraints.
## Derived from Digilent's Arty-A7-100-Master.xdc (fetched verbatim, see vivado/README.md):
## only the lines this design uses are uncommented; everything else is left as in the
## master file so the pin names can be cross-checked. The one rename: the master's
## CLK100MHZ port is called `clk` here (the pty harness and the RTL use that name).
...
## I2C to the INA219 on Pmod JA pins 1 (SCL) and 2 (SDA): open-drain from the FPGA,
## pull-ups on the breakout; enable the internal ones too so the bus idles high alone.
set_property PULLUP true [get_ports { ja[0] }]
set_property PULLUP true [get_ports { ja[1] }]

## The UART and switches are asynchronous to clk (synchronised inside): no timing paths.
set_false_path -from [get_ports { uart_txd_in sw[0] sw[1] ja[1] }]
set_false_path -to [get_ports { uart_rxd_out led[*] led0_r led0_g led0_b ja[*] }]

## QSPI flash boot (the on-board 16 MB S25FL128): 4-bit bus, faster config clock.
