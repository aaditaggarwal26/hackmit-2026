...
class Simulation:
    def __init__(
        self, scenario: Scenario, settings: Settings, seed: int, run_id: str | None = None, write_run: bool = True
    ) -> None:
        ...
        self.sat_bus = {
            sat.hostname: LoopbackBus(self.hub, sat.hostname, self.s.dedup_window, self.s.bus_max_datagram)
    ...
    def _observe(self, msg: M.Message) -> None:
        ...
        if isinstance(msg, M.Grant):
            ...
            row = SlotRow(
                ...
                winner=msg.to,
                item_id=msg.item_id,
                score=msg.breakdown.score,
                item_age_term=msg.breakdown.item_age_term,
                sat_wait_term=msg.breakdown.sat_wait_term,
                total=msg.breakdown.total,
                runner_up=ru.hostname if ru else "-",
                runner_total=ru.total if ru else None,
                margin=d.margin,
                excluded=sorted(d.excluded),
                sats=self._sat_columns(),
            )
    ...
    def _sat_columns(self) -> dict[str, dict[str, Any]]:
        ...
        for sat in self.sats:
            ...
            out[sat.hostname] = dict(
                occ=sat.buffer.stats().occupancy_pct,
                qlen=len(sat.queue),
    ...
    def table(self) -> str:
        ...
        w(
            f"seed={self.seed}  ITEM_AGING_RATE={self.s.item_aging_rate}  SAT_AGING_RATE={self.s.sat_aging_rate}  "
        ...
        for r in self.rows:
            ...
            line = (
                f"{r.round_id:>3} {r.t:>6.1f}  {r.winner:<6} {r.item_id:>5}  {r.score:>6.1f} "
            ...
            for h in hosts:
                c = r.sats.get(h)
                cols.append(
                    f"{c['occ']:>4.0f} {c['qlen']:>2} {c['ev']:>3}  {c['flag']:<10}" if c else f"{'offline':<22}"
                )
            w(line + "  ".join(cols) + "\n")
        w("\n")
        # per-satellite summary
        w(
            f"{'satellite':<8} {'captured':>8} {'granted':>7} {'sent':>5} {'evicted':>7} {'rejected':>8} {'failed':>6} "
        ...
        for sat in self.sats:
            ...
            a = flags.get(sat.hostname)
            w(
                f"{sat.hostname:<8} {cnt.captured:>8} {cnt.grants:>7} {cnt.transmitted:>5} {cnt.evicted:>7} "
        ...
        gc = g.counters
        w(
            f"ground: state={g.state} rounds={gc.rounds} grants={gc.grants} completed={gc.completed} "
        ...
        w(
            f"window: {ws['slots_used']}/{ws['slots_total']} slots used, {ws['remaining_bytes']} bytes left, "
            f"open={ws['open']}\n"
        )
        b = self.ground_bus.stats
        w(
            f"ground bus: received={b.received} delivered={b.delivered} dup={b.dropped_dup} "
            f"malformed={b.dropped_malformed} own={b.dropped_own}  hub datagrams={self.hub.datagrams}\n"
        )
        w(
            f"telemetry events: {len(self.events)}  bus log entries: {len(self.bus_log)}  "
            f"virtual time: {self.now:.1f}s\n"
        )
        st = self.stream
        w(
            f"event stream: {st.seq} events → {self.run_file.path if self.run_file else '(not written)'}  "
...
def _flag_str(kind: object, level: object, pressured: bool) -> str:
    ...
    base = {
        "nominal": "ok",
        "starved": f"starved/{lv}",
        "idle": "idle",
        "never_transmitted": f"new/{lv}",
        "silent": "SILENT",
    }.get(k, k)
